...
async function routeCore(request, response) {
  ...
  try {
    ...
    if (request.method === 'GET' && requestUrl.pathname === '/api/debug/account-pair-jonathan') {
      const targets = ['jonathan.vanderelst@outlook.com', 'jonathandelrey@outlook.com']
      const findTargetUsers = (email) => db.users.filter((user) => {
        const profile = user.profile ?? {}
        return [profile.email, profile.contact, profile.phone]
          .some((value) => String(value ?? '').trim().toLowerCase() === email)
      })
      const summarize = (email) => findTargetUsers(email).map((user) => {
        const profile = withDatingDefaults(user.profile ?? {})
        const verification = user.authSecret?.emailVerification
        const latestEmail = [...db.authEmails]
          .reverse()
          .find((mail) => mail.userId === user.id && mail.type === 'email_verification')
        return {
          id: user.id,
          name: profile.name,
          email: profile.email,
          ...
          authUserLinked: Boolean(user.authUserId),
          onboarded: Boolean(user.onboarded),
          deleted: Boolean(user.deletedAt),
          emailVerified: Boolean(profile.emailVerified),
          verification: verification ? (verification.usedAt ? 'used' : 'pending') : 'none',
          genderIdentity: profile.genderIdentity,
          interestedIn: profile.interestedIn,
          city: profile.city,
          completion: profile.profileCompletion,
          bioChars: String(profile.bio || profile.about || '').length,
          photos: Array.isArray(profile.photos) ? profile.photos.length : (profile.photo ? 1 : 0),
          sessionId: latestSessionId(db, user.id),
          latestEmail: latestEmail ? {
            provider: latestEmail.provider,
            delivered: Boolean(latestEmail.delivered),
            error: latestEmail.error || '',
          } : null,
        }
      })
      const usersByTarget = Object.fromEntries(targets.map((target) => [target, findTargetUsers(target)]))
      const first = (target) => usersByTarget[target].find((user) => !user.deletedAt) ?? usersByTarget[target][0]
      const exclusionReasons = (current, candidate) => {
        const reasons = []
        if (!current) reasons.push('current_missing')
        if (!candidate) reasons.push('candidate_missing')
        if (!current || !candidate) return reasons
        if (candidate.id === current.id) reasons.push('same_user_id')
        if (!candidate.onboarded) reasons.push('candidate_not_onboarded')
        if (candidate.deletedAt) reasons.push('candidate_deleted')
        if (isInternalTestUser(candidate)) reasons.push('candidate_internal_test')
        if (profilesShareContact(current.profile, candidate.profile)) reasons.push('profiles_share_contact')
        if ([...(db.hidden[current.id] ?? []), ...(db.blocks[current.id] ?? [])].includes(candidate.id)) reasons.push('candidate_hidden_or_blocked')
        if (!profilesCanMatch(current.profile, candidate.profile)) reasons.push('preferences_block')
        return reasons
      }
      const left = first(targets[0])
      const right = first(targets[1])
      const leftReasons = exclusionReasons(left, right)
      const rightReasons = exclusionReasons(right, left)
      const leftMatch = left && right && !leftReasons.length ? buildMatch(left, right, db) : null
      const rightMatch = left && right && !rightReasons.length ? buildMatch(right, left, db) : null
      send(response, 200, {
        ok: true,
        totals: {
        ...
        accounts: Object.fromEntries(targets.map((target) => [target, summarize(target)])),
        pair: {
          leftEmail: targets[0],
          rightEmail: targets[1],
          leftSeesRight: !leftReasons.length,
          leftReasons,
          rightSeesLeft: !rightReasons.length,
          rightReasons,
          shareContact: Boolean(left && right && profilesShareContact(left.profile, right.profile)),
          canMatch: Boolean(left && right && profilesCanMatch(left.profile, right.profile)),
          leftScoreForRight: leftMatch ? { score: leftMatch.score, discoveryScore: leftMatch.discoveryScore, distance: leftMatch.distance } : null,
          rightScoreForLeft: rightMatch ? { score: rightMatch.score, discoveryScore: rightMatch.discoveryScore, distance: rightMatch.distance } : null,
        },
      })
      return
    }
